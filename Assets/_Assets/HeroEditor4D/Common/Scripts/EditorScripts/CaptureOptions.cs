...
namespace Assets.HeroEditor4D.Common.Scripts.EditorScripts
{
    public class CaptureOptions : MonoBehaviour
    {
        ...
        public void Capture()
        {
            ...
            if (Slash.isOn) options.Add(new CaptureOption("Idle", FindFirstObjectByType<Character4D>().WeaponType == WeaponType.Melee2H ? "Slash2H" : "Slash1H"));
            ...
            FindFirstObjectByType<SpriteSheetCapture>().Capture(direction, options, int.Parse(FrameSize.text), int.Parse(FrameCount.text), Shadow.isOn);
            Close();
        }
